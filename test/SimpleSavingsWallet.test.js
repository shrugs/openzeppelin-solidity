...
contract('SimpleSavingsWallet', function (accounts) {
  ...
  it('should receive funds', async function () {
    await pweb3.eth.sendTransaction({ from: owner, to: savingsWallet.address, value: paymentAmount });
    const balance = await pweb3.eth.getBalance(savingsWallet.address);
  ...
  it('owner can send funds', async function () {
    ...
    await pweb3.eth.sendTransaction({ from: accounts[9], to: savingsWallet.address, value: 1000000 });
    ...
    const balance = await pweb3.eth.getBalance(accounts[1]);
    await savingsWallet.sendTo(accounts[1], paymentAmount, { from: owner });
    const updatedBalance = await pweb3.eth.getBalance(accounts[1]);
