...
contract('SplitPayment', function ([owner, payee1, payee2, payee3, nonpayee1, payer1]) {
  ...
  it('should accept payments', async function () {
    await pweb3.eth.sendTransaction({ from: owner, to: this.contract.address, value: amount });

    const balance = await pweb3.eth.getBalance(this.contract.address);
  ...
  it('should throw if non-payee want to claim', async function () {
    await pweb3.eth.sendTransaction({ from: payer1, to: this.contract.address, value: amount });
  ...
  it('should distribute funds to payees', async function () {
    ...
    // receive funds
    const initBalance = await pweb3.eth.getBalance(this.contract.address);
    initBalance.should.be.bignumber.equal(amount);

    // distribute to payees
    const initAmount1 = await pweb3.eth.getBalance(payee1);
    await this.contract.claim({ from: payee1 });
    const profit1 = await pweb3.eth.getBalance(payee1) - initAmount1;
    assert(Math.abs(profit1 - web3.toWei(0.20, 'ether')) < 1e16);

    const initAmount2 = await pweb3.eth.getBalance(payee2);
    await this.contract.claim({ from: payee2 });
    const profit2 = await pweb3.eth.getBalance(payee2) - initAmount2;
    assert(Math.abs(profit2 - web3.toWei(0.10, 'ether')) < 1e16);

    const initAmount3 = await pweb3.eth.getBalance(payee3);
    await this.contract.claim({ from: payee3 });
    const profit3 = await pweb3.eth.getBalance(payee3) - initAmount3;
    assert(Math.abs(profit3 - web3.toWei(0.70, 'ether')) < 1e16);

    // end balance should be zero
    const endBalance = await pweb3.eth.getBalance(this.contract.address);
